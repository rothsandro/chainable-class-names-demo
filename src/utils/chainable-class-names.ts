...
declare type CssModuleCondition<M extends CssModuleObj> = (condition: unknown) => CssModuleChain<M>;
declare type CssModuleClasses<M extends CssModuleObj> = { [key in keyof M]: CssModuleCallableChain<M> };
declare type CssModuleChain<M extends CssModuleObj> = CssModuleClasses<M> &
  CssModuleOutput &
  CssModuleFnOutput;
declare type CssModuleCallableChain<M extends CssModuleObj> = CssModuleChain<M> & CssModuleCondition<M>;
...
function ccn<M extends CssModuleObj>(mod: M): CssModuleClasses<M> {
  ...
  function conditionFn(condition?: unknown) {
    ...
    condition ? classNames.add(latestClassName) : classNames.delete(latestClassName);
    latestClassName = "";
    return chainProxy;
  }
  ...
  const callableChainProxy = new Proxy<CssModuleCallableChain<M>>(conditionFn as any, handler);
  const chainProxy = new Proxy<CssModuleChain<M>>(buildFinalClassName as any, handler);
